...
3. 원문 텍스트를 실제로 확보하지 못했으면 그 문헌의
   ...
   를 적습니다.
4. A/B/C 분류가 원문으로 검증되지 않았다면 그 문헌에 `잠정 분류`라고 명시
   하십시오.
...
- 문헌번호와 명칭·출원인은 같은 후보 전용 페이지에서 함께 확인한 경우에만
  적으십시오. ARIA 는 후보의 문헌번호가 그 url 안에 실제로 들어 있는지
  대조하며, 대조되지 않으면 명칭·출원인·패밀리를 출력에서 제외합니다. 번호만
...
- counterpart 에는 support_text 가 실제로 말하는 것만 적으십시오. "~로 볼 수
  ...
  쓰지 마십시오. 추론은 different 칸에 "문헌에 없는 부분" 으로 적으십시오.
- A/B/C 분류에는 support_source 가 page_text 인 행이 최소 하나 필요합니다.
  페이지를 열지 못한 후보는 그룹에 넣지 말고 미확인 단서로 남기십시오.
...
# 에 해당하는 도구가 없다. 셸로 내려받는 것은 도구 정책 위반이다.
#
# 그래서 이 실행에서 얻을 수 있는 증거는 검색 스니펫이 상한이다. 없는 능력을
# 전제한 문구를 그대로 두면 모델이 열어 보지도 않은 페이지에
# source_page_reviewed 를 붙인다.
#
...
_CODEX_CONTEXT_EDITS = (
    (
        """- WebSearch 와 WebFetch 두 가지뿐입니다. 다른 도구는 제공되지 않으며, 파일
  읽기·쓰기·명령 실행을 시도하지 마십시오.""",
        """- web_search 하나뿐입니다. 이 실행에는 페이지를 여는 도구가 없습니다. 파일
  읽기·쓰기·명령 실행을 시도하지 마십시오. 시도하면 ARIA 가 탐지해 이 실행을
  실패로 처리합니다.""",
    ),
    (
        """WebFetch 는 페이지 원문을 그대로 주는 도구가 아닙니다. 페이지를 변환한 뒤
...
2. WebFetch 출력 문장을 특허·논문의 직접 인용문처럼 표시하지 마십시오.
   따옴표로 묶어 원문 인용처럼 제시하는 것도 안 됩니다.""",
        """이 실행에는 페이지를 여는 도구가 없습니다. 당신이 볼 수 있는 것은 web_search
가 돌려주는 검색 결과와 스니펫뿐입니다. 따라서 다음을 지키십시오.

1. 검색 결과 제목, 스니펫, 자동 요약은 후보 탐색 자료로만 사용합니다.
2. 스니펫 문장을 특허·논문의 직접 인용문처럼 표시하지 마십시오. 따옴표로 묶어
   원문 인용처럼 제시하는 것도 안 됩니다.""",
    ),
    (
        "3. 관련성이 높은 후보의 실제 페이지를 WebFetch 로 확인 시도",
        "3. 관련성이 높은 후보는 검색어를 바꿔 다시 web_search 해서 서지사항 교차 확인",
    ),
    (
        """- url 에는 그 후보를 확인한 실제 주소를 적으십시오. ARIA 는 이 주소를 당신이
  실제로 성공시킨 WebFetch 호출과 대조합니다. 열어 보지 않은 주소에
  source_page_reviewed 를 붙이면 자동으로 candidate_only 로 내려갑니다.""",
        """- url 에는 그 후보를 확인한 실제 주소를 적으십시오. 이 실행에는 페이지를 여는
  도구가 없으므로 source_page_reviewed 를 붙일 수 없습니다. 붙여도 ARIA 가
  자동으로 candidate_only 로 내립니다.""",
    ),
    (
        """    search_snippet      검색 결과 제목·스니펫만 봤다
    webfetch_summary    WebFetch 로 페이지를 열어 요약을 받았다
  ...
  WebSearch/WebFetch 만으로는 raw_original_verified 를 부여하지 마십시오.""",
        """    search_snippet      검색 결과 제목·스니펫만 봤다
  이 실행에서 쓸 수 있는 값은 search_snippet 하나뿐입니다. 페이지를 여는 도구가
  없으므로 webfetch_summary 나 raw_original_verified 를 부여하지 마십시오.""",
    ),
    (
        '- evidence_status 는 "candidate_only" 또는 "source_page_reviewed" 입니다.',
        '- evidence_status 는 이 실행에서 항상 "candidate_only" 입니다.',
    ),
)
