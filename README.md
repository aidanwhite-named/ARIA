...
| Gemini (agy) | **사용 가능** | agy 1.1.15 로 종단 검증 완료. 도구를 끌 수 없다는 제약이 있습니다. |
| Claude (Claude Code) | 설치됨 · 로그인 필요 | `claude setup-token` 실행 후 사용 가능합니다. |
...
**agy 의 두 가지 제약을 알고 쓰십시오.**

| 제약 | 영향 |
|---|---|
| `--system-prompt` 에 해당하는 플래그가 없음 | ARIA 런타임 컨텍스트가 사용자 메시지 맨 앞에 붙습니다. 첨부 본문과 같은 층위라서 프롬프트 인젝션 방어가 Claude 보다 약합니다. |
| `--tools` 에 해당하는 플래그가 없음 | `run_command`, `write_to_file` 을 포함해 57개 도구가 활성 상태로 실행됩니다(`permission_mode: request-review`). ARIA 는 이 사실을 매 실행 경고로 남기고, **실제 도구 호출이 감지되면 기본적으로 실패 처리**합니다. |

ARIA 는 `--dangerously-skip-permissions` 를 절대 사용하지 않습니다.
...
- **Gemini(agy) 의 도구 차단** — agy 에는 도구를 끄는 플래그가 없습니다. ARIA 는
  실제 호출을 감지해 실패 처리하지만, Claude 처럼 애초에 도구를 없애지는 못합니다.
- **Gemini(agy) 의 시스템 프롬프트 분리** — 불가능합니다. 런타임 컨텍스트가 사용자
  메시지에 포함되어 인젝션 방어가 약합니다.
