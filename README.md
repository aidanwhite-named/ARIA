...
**A**uditable **R**untime for **I**nvention **A**nalysis — 선택한 Master Prompt 를
선택한 AI CLI 에서 안전하고 일관되게 실행하고, 입력부터 결과까지 감사 가능한 흔적을
남기는 로컬 발명 분석 워크스페이스.
...
요건이 아니다. 대신 경고를 남기고 「종속항 추가 분석」 버튼만 잠근다.
...
이전 보고서를 넣지 않으므로 보고서가 「이전 분석과 달라진 부분」을 스스로 적을
수 없다. 그 자리는 **원본과 비교** 화면이 메운다. 두 보고서를 나란히 놓고 줄이
다른 곳만 표시한다.

ARIA 는 어느 쪽이 옳은지, 어떤 차이가 중요한지 판단하지 않는다. 판단은 모델이,
대조는 사람이 한다. 그래서 "ARIA 는 분석 방법을 갖지 않는다"는 원칙이 유지된다.
...
없다. 실제로 확인하려면 **원본과 비교** 로 두 보고서를 대조하는 수밖에 없다.
발췌문이 토씨까지 같은데 「이번에 추가·변경된 청구항」만 채워져 있다면 의심할
만하다.
